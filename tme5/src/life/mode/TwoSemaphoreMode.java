...
public class TwoSemaphoreMode implements LifeMode {
    ...
    public void startSimulation(LifeModel model, LifePanel panel, AtomicInteger updateDelayMs, AtomicInteger refreshDelayMs,
        ...
        for (int i = 0; i < n; i++) {
            ...
            Thread updater = new Thread(new Updater(model, startRow, endRow, ready, done), "updater-" + i);
            updater.start();
        }
    ...
    static final class Refresher implements Runnable {
        ...
        Refresher(LifeModel model, AtomicInteger delayMs, LifePanel panel, SimpleSemaphore ready, SimpleSemaphore done, int n) {
            this.model = model;
